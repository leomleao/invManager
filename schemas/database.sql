...
CREATE  TABLE IF NOT EXISTS `invManager`.`users` (
  ...
  `userLastKnownPassword` CHAR(128) NULL ,
...
CREATE TABLE `invManager`.`items` (
 ...
 `itemAllocatedToUserID` INT NOT NULL COMMENT 'who helds the item' ,
 `itemDescription` INT NOT NULL , 
 `itemSpec` INT NOT NULL COMMENT 'processor RAM etc', 
 `itemName` INT NULL COMMENT 'PC name for instance', 
 `itemInvoiceNumber` INT NOT NULL  COMMENT 'NF number', 
 `itemInventoryCode` INT NOT NULL  COMMENT 'NIF if existent',
 `itemPrice` DOUBLE NOT NULL , 
 `itemPicturePath` VARCHAR(90) NULL , 
 `itemPurchaseDate` DATE NOT NULL , 
 `itemMAC` VARCHAR(12) NOT NULL ,
 `itemSerialNumber` VARCHAR(45) NOT NULL ,
 `itemPartNumber` VARCHAR(45) NOT NULL ,
 `itemIMEI` VARCHAR(45) NOT NULL ,
